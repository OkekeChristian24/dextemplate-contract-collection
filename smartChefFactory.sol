...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when Bswap mining ends.
    ...
    // Deposit fee in percent
    uint256 public depositFee;

    // Withdrawal fee in percent
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            uint256 depFee = _amount * depositFee / 100;
            _amount = _amount.sub(depFee);
            if(depFee > 0){
                stakedToken.safeTransferFrom(address(msg.sender), devaddr, depFee);
            }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 withFee = _amount * withdrawalFee / 100;
            _amount = _amount.sub(withFee);
            if(withFee > 0){
                stakedToken.safeTransfer(devaddr, withFee);
            }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 bswapReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(bswapReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 bswapReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(bswapReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
